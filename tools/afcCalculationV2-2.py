...
class AFC_project:
	...
	def __init__(self,file_path="./标准输入.xlsx",tab_names=["近期早高峰客流","近期晚高峰客流"],parameter_list=[20,24,6,8,5,4,20,20]):
	...
	def run(self):
		...
		df_jp_numpy[:,2]
		output1 = pd.concat([df_jq.iloc[:,[0,2,4,6,8,10,12,14,16]],jqzgfzj,jqwgfzj,jqzgfcj,jqwgfcj,pd.DataFrame(F,index=df_jq.index),pd.DataFrame(G,index=df_jq.index),
...
# 按照最大高峰小时人流(含超高峰系数)计算所需检票机数量
		...
		output3 = pd.concat([
			...
			pd.DataFrame(AA,index=df_jq.index),
			pd.DataFrame(AB,index=df_jq.index),
			pd.DataFrame(AC,index=df_jq.index),
			pd.DataFrame(AD,index=df_jq.index),
			...
			pd.DataFrame(Q,index=df_jq.index),
			pd.DataFrame([K]*len(Z),index=df_jq.index)],
			axis=1)

		output3.columns = ["进站检票机","出站检票机","双向检票机","宽通道双\n向检票机",
		"可用的出站\n检票机数量","检票机出站\n通过时间(min)",
		"乘客疏散后\n空闲时间(s)\n(此值越小越拥堵)","自动售票机\n数量","列车间隔\n时间(min)",
		"高峰小时平均\n一列车的出站\n人数(含超高峰系数)",
		"出站检票机\n计算数量(MAX)","检票机通过率\n(人/分钟)"]
