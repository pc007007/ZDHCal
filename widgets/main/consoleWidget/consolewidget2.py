...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QPushButton, QPlainTextEdit, QApplication
from PySide6.QtGui import QSyntaxHighlighter, QTextCharFormat, QColor, QFont, QTextCursor, QScreen
from PySide6.QtCore import Qt, Signal, QProcess, QThread, QRegularExpression

class PlainTextEdit(QPlainTextEdit):
    commandSignal = Signal(str)
    commandZPressed = Signal(str)
...
class name_highlighter(QSyntaxHighlighter):
    ...
    def __init__(self, parent=None, user_name=None, host_name=None, cwd=None):
        ...
        for f in most_used:
            ...
            self.highlightingRules.append((QRegularExpression("\\b" + f + "\\b"), nameFormat))

        hostnameFormat = QTextCharFormat()
        hostnameFormat.setForeground(QColor("#12c2e9"))
        self.highlightingRules.append((QRegularExpression(self.name), hostnameFormat))
        self.highlightingRules.append((QRegularExpression(self.name2), hostnameFormat))

        otherFormat = QTextCharFormat()
        otherFormat.setForeground(QColor("#f7797d"))
        self.highlightingRules.append((QRegularExpression("~\/[^\s]*"), otherFormat))

        quotation1Format = QTextCharFormat()
        quotation1Format.setForeground(QColor("#96c93d"))
        self.highlightingRules.append((QRegularExpression("\"[^\"]*\""), quotation1Format))

        quotation2Format = QTextCharFormat()
        quotation2Format.setForeground(QColor("#96c93d"))
        self.highlightingRules.append((QRegularExpression("'[^\']*\'"), quotation2Format))

        integerFormat = QTextCharFormat()
        integerFormat.setForeground(QColor("#cc5333"))
        integerFormat.setFontItalic(True)
        self.highlightingRules.append((QRegularExpression("\\b[-+]?[0-9]+\\b"), integerFormat))
